...
> "Lee las instrucciones del proyecto. Estamos en la Fase 0. Crea el proyecto Django llamado 'sige' con una app 'core'. Necesito: modelo de Usuario personalizado extendiendo AbstractUser con los campos adicionales del CLAUDE.md, modelo de Rol y Permiso con permisos granulares por módulo y operación, middleware de auditoría que registre automáticamente cada acción (usuario, acción, módulo, entidad, valor anterior, valor nuevo, timestamp, IP). Configura DRF con JWT. Al terminar, muéstrame los comandos de terminal que debo ejecutar."

## Carpeta canónica limpia — "eSYNAPSE 360 GitHub/"

Dentro de la carpeta de trabajo existe `eSYNAPSE 360 GitHub/`: el sistema SOLO con los
archivos necesarios y **siempre actualizado**, listo para instalar/publicar (GitHub).

Reglas permanentes:
- La carpeta de trabajo (`eSYNAPSE 360`) es el **taller**: pruebas, scripts temporales,
  diagnósticos, copias de respaldo, etc.
- **Cada cambio válido al sistema se REPLICA a `eSYNAPSE 360 GitHub/`** (la versión final del
  archivo modificado), para que esa carpeta siempre refleje el estado bueno y actualizado.
- En la carpeta limpia **NUNCA** va basura: nada de `__pycache__`, `venv`, `node_modules`,
  `dist`, `db.sqlite3`/`*.sqlite3`, `media/`, `certificados/`, `backups/`, `*.log`, `*.bak`,
  scripts de prueba/diagnóstico (`diagnostico_firma.py`, `limpiar_documentos_prueba.py`,
  `renombrar_sgi_a_sig.py`), ni temporales.
- El git del taller **ignora** `eSYNAPSE 360 GitHub/` (no la versiona; es un snapshot de fuente
  limpia, no un sub-repo).
