...
const ICONO_MODULO = {
  'Hallazgos': '🔎', 'Acciones Correctivas': '🔧', 'Documentos': '📄', 'Auditorías': '📋', 'Equipos': '📐',
}
...
export default function Layout() {
  ...
  const cargarTareas = () => {
    api.misTareas().then((d) => {
      const t = d.tareas || []
      setTareas(t)
      // Poda las "leídas" a las tareas que siguen existiendo (evita crecer sin fin)
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-14 shrink-0 items-center justify-end gap-3 border-b border-slate-200 bg-white px-4 dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="relative" ref={buzonRef}>
            ...
            {abierto && (
              <div className="absolute right-0 top-11 z-50 max-h-96 w-80 overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-xl dark:border-slate-800 dark:bg-slate-900">
                ...
                {noLeidas.length === 0 ? (
                  <p className="px-3 py-6 text-center text-xs text-slate-400">Estás al día 🎉</p>
                ) : (
                  <ul className="divide-y divide-slate-100 dark:divide-slate-800">
                    {noLeidas.slice(0, 8).map((t, i) => (
                      <li key={i}>
                        <button onClick={() => irA(t.ruta, t)} className="flex w-full items-start gap-2 px-3 py-2 text-left transition hover:bg-slate-50 dark:hover:bg-slate-800/60">
                          <span className="mt-0.5 text-base leading-none">{ICONO_MODULO[t.modulo] || '•'}</span>
                          <span className="min-w-0 flex-1">
                            ...
                            {t.fecha && <span className="text-[10px] text-amber-600 dark:text-amber-400">📅 {t.fecha}</span>}
                          </span>
                        </button>
                      </li>
                    ))}
